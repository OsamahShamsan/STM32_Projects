...
static void uart_start_next_message(UART_HandleTypeDef* huart);

// Blocking send with chunking
bool uart_send_blocking(UART_HandleTypeDef* huart, const char* str) {
    if (!huart || !str) return false;
...
bool uart_send_async(UART_HandleTypeDef* huart, const char* str) {
    if (!huart || !str || uart_tx_busy) return false;
...
static void uart_start_next_message(UART_HandleTypeDef* huart) {
    ...
    str_sent += chunk_len;
    HAL_UART_Transmit_IT(huart, tx_buffer, (uint16_t)chunk_len);
}
...
void uart_writer_test_hi_red(UART_HandleTypeDef* huart) {
    ...
    vt100_get_set_color(color_buf, sizeof(color_buf), VT100_RED);
    vt100_get_reset_color(reset_buf, sizeof(reset_buf));

    uart_send_blocking(huart, color_buf);
    uart_send_blocking(huart, "HI\r\n");
    uart_send_blocking(huart, reset_buf);
}
*/
